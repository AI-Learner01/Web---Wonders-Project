...
function SummaryCard({ selectedPackage, bookingData }) {
    ...
    const handlePayment = async () => {
        ...
        try {
            ...
            if (data.success) {
                ...
                setModalMessage(`Your booking details have been sent to ${bookingData.email}.`);
                setShowSuccess(true);
            } else {
                // Update state to show error modal
                setModalMessage(`Failed to send the email: ${data.message}`);
                setShowError(true);
            }
        } catch (err) {
            console.error(err);
            // Show error modal for network/server errors
            setModalMessage("Error connecting to the server to send email.");
            setShowError(true);
        } finally {
    ...
    return (
        <>
            <div className="sticky top-6 rounded-[32px] p-7 bg-white/70 backdrop-blur-2xl border border-white/40 shadow-[0_25px_50px_rgba(0,0,0,0.12)] transition-all duration-500 hover:-translate-y-1">
                ...
                <div className="space-y-3 mb-6">
                    ...
                    <button
                        ...
                        className={`mt-4 w-full rounded-2xl py-4 font-bold text-lg text-white shadow-xl transition-all duration-300 ${isProcessing
            ...
            {/* MODALS RENDERED OUTSIDE THE MAIN CARD SO THEY CENTER ON THE WHOLE SCREEN */}
            <SuccessModal
                isOpen={showSuccess}
