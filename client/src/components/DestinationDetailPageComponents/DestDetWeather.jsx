...
// Safe date formatter to prevent UTC timezone shifts
...
const DestDetWeather = ({ locationName }) => {
  ...
  useEffect(() => {
    const fetchWeatherData = async () => {
      // FIX 1: If locationName isn't available yet, stop loading so it doesn't get stuck forever
      if (!locationName) {
        setLoading(false);
        return;
      }
      ...
      try {
        const res = await fetch(
          `${import.meta.env.VITE_SERVER_URL}/api/destinations/weather?name=${encodeURIComponent(locationName)}`
        );
  ...
  // FIX 2: Render explicit error message instead of returning null
  if (error) {
    return (
      <div className="my-8 rounded-3xl bg-rose-50/60 border border-rose-200/60 p-6 text-center text-rose-600 shadow-sm">
        <p className="text-sm font-semibold">{error}</p>
      </div>
    );
  }

  // Safe check for weather structure
  if (!weather || !weather.current || !weather.forecast || weather.forecast.length === 0) {
    return null;
  }

  const isToday = selectedDayIndex === 0;
  const activeForecast = weather.forecast[selectedDayIndex] || weather.forecast[0];
  ...
  const displayInfo = isToday
    ...
    : {
        title: formatDateLabel(activeForecast.date),
        weatherCode: activeForecast.weatherCode,
        tempDisplay: `${activeForecast.maxTemp}° / ${activeForecast.minTemp}°C`,
        subTemp: `High ${activeForecast.maxTemp}°C • Low ${activeForecast.minTemp}°C`,
        metricLabel: "Rain Chance",
      ...
      <section className="relative z-10 p-6 sm:p-10 bg-white/60 backdrop-blur-2xl border border-white/70 rounded-3xl text-slate-800 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
        ...
        <div className="flex flex-col lg:flex-row items-center justify-between gap-8 pb-8 border-b border-slate-200/50">
          ...
          <div className="text-center lg:text-left">
            ...
            <h3 className="text-4xl sm:text-5xl font-extrabold mt-3 text-slate-900 tracking-tight">
              {weather.locationName}
            </h3>
            <p className="text-slate-500 text-sm font-medium mt-2">
              {displayInfo.title} • {activeWeather.label}
            </p>
          </div>

          {/* Center: Main Temperature */}
          <div className="flex items-center gap-4 sm:gap-6">
            <span className="text-6xl sm:text-7xl transition-transform duration-500 hover:scale-110 hover:-rotate-6 drop-shadow-md">
              {activeWeather.emoji}
            </span>
            <div className="text-left">
              <span className="text-5xl sm:text-6xl font-black text-slate-800 tracking-tighter">
                {displayInfo.tempDisplay}
              </span>
        ...
        <div className="mt-8">
          <div className="flex items-center justify-between mb-5">
            <h4 className="text-[10px] sm:text-xs font-bold uppercase tracking-widest text-slate-500">
              5-Day Forecast{" "}
              <span className="text-slate-400 font-medium tracking-normal normal-case ml-2 hidden sm:inline-block">
            ...
            {weather.forecast.slice(0, 5).map((day, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-4xl my-3 drop-shadow-sm">
                    {dayInfo.emoji}
                  </span>
                  <span className="text-sm font-bold text-slate-700">
                    {day.maxTemp}°{" "}
