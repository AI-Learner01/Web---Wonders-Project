...
const DestDetAbout = ({aboutText}) => {
    ...
    const formatText = (text) => {
        ...
        return paragraphs.map((para, index) => (
            <p key={index} className="mb-5 leading-relaxed text-gray-700 text-[19px] text-justify">
                {index === 0 ? (
                    // Add a beautiful "Drop Cap" to the first letter of the first paragraph
                    <span className="float-left mr-2 mt-1 text-5xl font-extrabold text-emerald-500 leading-none">
                        {para.charAt(0)}
                    </span>
                ) : null}
