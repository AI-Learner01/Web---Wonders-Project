...
const { ObjectId } = require("mongodb");
const resend = require("../config/mail"); // 👈 Updated to Resend client
const { 
  collectionQuries, 
  collectionOtps, 
  collectionPackages 
...
const updateQueryStatus = async (req, res) => {
  try {
    ...
    try {
      console.log(`Sending query resolution email via Resend to: ${query.contact}...`);

      const info = await resend.emails.send({
        from: 'AuraAvenue Support <onboarding@resend.dev>', // Resend testing email format
        to: query.contact,
        subject: `Query Resolved - [${query.topic}]`,
        html: `
          <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 550px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 12px; overflow: hidden; background-color: #ffffff; color: #1e293b;">
      ...
      console.log("Resolution email sent successfully via Resend! Data:", info);
    } catch (mailError) {
      console.error("❌ Resolution Email Error:", mailError.message);
    }
