...
async function fetchLocation(locationName) {
    try {
        ...
        // STEP 1: ALWAYS try the exact title match first! (Fixes Ayodhya, Vadodara, Vrindavan)
        let wikiUrl = `https://en.wikipedia.org/w/api.php?action=query&format=json&prop=extracts|pageimages|description|coordinates&titles=${encodeURIComponent(queryTitle)}&explaintext=true&exsentences=10&pithumbsize=1920&redirects=1`;
        ...
        // STEP 2: Check if exact match failed, or if it returned something non-travel related (like the "Arsenal" weapon)
        ...
        if (isIrrelevant) {
            ...
            if (searchRes.data?.query?.search?.length > 0) {
                ...
                wikiUrl = `https://en.wikipedia.org/w/api.php?action=query&format=json&prop=extracts|pageimages|description|coordinates&titles=${encodeURIComponent(newTitle)}&explaintext=true&exsentences=10&pithumbsize=1920&redirects=1`;
